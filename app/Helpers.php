...
function format_int(int $number, string $type = null)
{
    if ($number > 999999999) {
        ...
        return number_format($number / 1000000, 1).($type == 'file' ? 'mb' : 'm');
    } elseif ($number > 999) {
        return number_format($number / 1000, 1).($type == 'file' ? 'kb' : 'k');
    } else {
        return $number.($type == 'file' ? 'b' : '');
    }
}
...
function tab($tab)
{
    return __(session('current_tab') == $tab ? 'text-blue-500' : '');
}
